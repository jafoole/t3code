...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.pallet/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.pallet"
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Pallet server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.pallet/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.pallet"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
