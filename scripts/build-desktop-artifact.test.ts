...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Pallet (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Pallet (Nightly)");
  });
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.protocols, [
        // Pallet rebrand: protocol display name diverges from upstream here.
        { name: "Pallet", schemes: ["t3code", "t3code-dev"] },
      ]);
